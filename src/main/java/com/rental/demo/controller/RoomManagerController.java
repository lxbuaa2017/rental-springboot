...
public class RoomManagerController {
    ...
    @ResponseBody
    public String deleteRoom(@RequestBody Room room){   //根据roomId删除
        String roomId=room.getRoomId();
        if(roomRepository.findById(roomId)==null)
            return JsonResult.build(ERROR,"该房源不存在",null);
        else {
            roomManagerService.removeRoom(room);
            return JsonResult.build(SUCCESS,"已成功删除该房源",null);
        }
    ...
    @ResponseBody
    public String updateRoom(@RequestBody Room room){           //根据roomId更新信息
        String roomId=room.getRoomId();
        if(roomRepository.findById(roomId)==null)
            return JsonResult.build(ERROR,"该房源不存在",null);
        else {
            roomManagerService.updateRoom(room);
            return JsonResult.build(SUCCESS, "已成功更新房源信息", null);
        }
    }
    @CrossOrigin(origins = "http://localhost:8080", allowCredentials = "true")
